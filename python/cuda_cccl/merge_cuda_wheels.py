...
def merge_wheels(wheels: List[Path], output_dir: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for i, wheel_dir in enumerate(extracted_wheels):
            # Extract CUDA version from wheel name
            wheel_name = wheels[i].name
            cuda_version = wheel_name.split(".cu")[1].split(".")[0]

            print(f"Processing wheel for CUDA {cuda_version}")
                ...
                for file in files:
                    if file.endswith((".so", ".dll", ".pyd")):
                        ...
                        if (
                            ...
                            # For CUDA libraries, create version-specific subdirectories
                            # The structure should be: cuda/cccl/parallel/experimental/cu12/
                            if (
                                len(parts) >= 4
                                and parts[2] == "parallel"
                                and parts[3] == "experimental"
                            ):
                                # Check if this is already a version-specific path
                                if len(parts) >= 5 and parts[4].startswith("cu"):
                                    # Already has version, just use the original path
                                    dest_path = base_wheel / rel_path
                                else:
                                    # Insert cuda version directory after experimental
                                    new_parts = (
                                        parts[:4] + [f"cu{cuda_version}"] + parts[4:]
                                    )
                                    dest_path = base_wheel / Path(*new_parts)
                            else:
                                # For other cuda/cccl paths, just use the original path
                                dest_path = base_wheel / rel_path
                        else:
                            dest_path = base_wheel / rel_path
